...
// Add this function to request certificates from the server
async function handleCmeClaimSubmission(event) {
  event.preventDefault(); // Prevent default form submission
  console.log("CME Claim Form submitted - processing (Google Apps Script Version)...");
  ...
  try {
      ...
      // --- 4. Handle Response and Update Firestore with Link ---
      cleanup(false, false); // Hide loader, keep buttons disabled initially

      if (scriptResult && scriptResult.downloadUrl) {
          // SUCCESS from Google Apps Script
          ...
          try {
              ...
              if (userDocSnap.exists()) {
                  ...
                  if (entryIndex > -1) {
                      // Add the download URL and certificate ID to the found entry
                      history[entryIndex].downloadUrl = scriptResult.downloadUrl;
                      history[entryIndex].certificateId = scriptResult.certificateId; // Save certificate ID
                      history[entryIndex].verificationHash = scriptResult.verificationHash; // Save verification hash

                      // Generate filename from user name if not provided
                      const fileName = `${certificateFullName.replace(/\s+/g, '_')}_CME_Certificate.pdf`;
          ...
          if (errorDiv) {
              ...
              errorDiv.innerHTML = `
                  ...
                  <a href="${scriptResult.downloadUrl}" target="_blank" class="auth-primary-btn" style="display: inline-block; margin-top: 10px; text-decoration: none; padding: 10px 15px; font-size: 1em;">
                      Download Certificate (PDF)
                  </a>
                  <p style="font-size: 0.85em; margin-top: 15px; color: #555;">Certificate ID: ${scriptResult.certificateId || 'N/A'}</p>
                  <p style="font-size: 0.85em; margin-top: 5px; color: #555;">A record of this claim has been saved to your history.</p>
              `;
          ...
          // FAILURE from Google Apps Script or fetch call
          const errorMessage = scriptResult && scriptResult.error ? scriptResult.error : 'Unknown error communicating with certificate service.';
      ...
      // --- 5. Refresh Dashboard (runs regardless of cert success/failure) ---
...
// --- FINAL handleCmeClaimSubmission Function (Generate & Download Link Version) ---
    ...
    try {
        ...
        if (cmeHistory.length > 0) {
          ...
          cmeHistory.forEach(claim => {
              ...
              } else if (claim.timestamp instanceof Date) { // Handle potential client-side Date objects if any exist
                  claimDate = claim.timestamp.toLocaleDateString();
              }

              // --- Check for downloadUrl and add button/link ---
              let downloadCellContent = '-'; // Default if no link
              if (claim.downloadUrl) {
                  // Use an anchor tag styled as a small button
                  downloadCellContent = `
